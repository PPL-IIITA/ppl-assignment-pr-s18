...
Select2::Select2 (int kcc, Couple cl[50]) {
	k = kcc;
	int i;
	for (i = 0; i < k; i++) {
		cp[i] = cl[i];
...
void Select2::way2 (Gift gift[100], int ngf) {
	int l = 0, i, j;
	...
	for (i = 0; i < k; i++) {
		...
		for (j = 1; j < cpgift[i][0].price; j++) {
			if (cpgift[i][j].type == 0) {
				flag1 = 1;
			}
			else if (cpgift[i][j].type == 1) {
				flag2 = 1;
			}
			else if (cpgift[i][j].type == 2) {
				flag3 = 1;
			}
		}
		if (flag1 == 0) {
			for (j = 1; j < ngf; j++) {
				if (gift[j].type == 0) {
					cpgift[i][0].price += 1;
					cpgift[i][(cpgift[i][0].price)-1] = gift[j];
					break;
				}
			}
		}
